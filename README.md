...
7. Using Minikube with Docker as the driver, start a Kubernetes cluster with 3 worker nodes running Kubernetes version 1.27.3 by running the following command.
   ```sh
   ~$ minikube start --driver=docker -p argocd-workshop
   ```
   > This process might take some time if it is the first start up of minikube.

8. Install ArgoCD in the Kubernetes cluster by running the following commands.
   ```sh
...
9. Run one of the following commands to serve the ArgoCD service on your local machine in a different terminal instance. Whatever URL (127.0.0.1:port) you get from this we will call argoURL.
    ```sh
